...
def extract_salesperson_phrase(text: str) -> str | None:
    ...
    if m:
        cand = m.group(1).strip()
        # Split candidate on common query/filler words that mark the end of the name
        cand = re.split(
            r"\b(?:in|from|between|who|with|that|has|had|having|did|does|made|by|for|to)\b",
            cand,
            maxsplit=1,
            flags=re.I,
        )[0].strip()
        
        # If the candidate itself is a filler or summary term, ignore it
        if cand.lower() in (
            "performance",
            "target",
            "achievement",
