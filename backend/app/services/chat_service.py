...
from app.intent_detection.entities import extract_customer_phrase, extract_salesperson_phrase, extract_years
...
def handle_chat_message(db: Session, engine, message: str, history: list[ChatMessage]) -> ChatResponse:
    ...
    # If confidence is too low to trust rule-based matching, ask the LLM for refinement
    if detected.confidence < 0.55 and settings.openrouter_api_key:
        llm = refine_intent_with_llm(trimmed)
        detected = _merge_llm(detected, llm)

    # If even after LLM refinement the intent is not a standard lookup, fall back to Text-to-SQL
    if detected.intent not in ("chitchat", "sales_summary") and detected.confidence < 0.75:
